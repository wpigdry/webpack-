...
const defaultConfig = {
    ...
    plugins: [
        ...
        // 解析html 并自动引入js
        // HtmlWebpackPlugin 这个插件，new 一个，就打包一个 HTML 页面，在 plugins 配置里 new 两个，就能打包出两个页面来。
        ...
        new HtmlWebpackPlugin({
            ...
            template: 'public/index.html',
            // 用于设置打包输出后的html模板名称
            filename: 'index2.html', // 需要设置成public的html文件同名，才可以把打包的js链接到html
            // 设置 js 这个打包文件是否插入html
            // false会删除html中--webpack打包后引入的script标签(src指向打包后的js)，但不会删除html本身自有的script标签
            inject: true, // 默认为true  
            // 配置压缩规则
             ...
             * @app2 打包的app2.js文件
             */
            chunks: ['vendor', 'common', 'app2']
        }),
        // 关于多入口，还可以把入口文件统一放在一个文件夹下，创建多入口配置文件，利用fs模块解析文件夹，chunks进行区分配置，
        // 数组循环new HtmlWebpackPlugin(),然后把配置数组导出exports，使用maerge合并其他的配置，在plugins下concat合并这个导出的plugin配置
        new VuePlugin(),
